...
public class TransactionalWorker {
    ...
    public void assertAllEntitiesHaveBeenPersisted() {
        ...
        for (TestEntity storedEntity : storedEntities) {
            assertNotNull("At least one entity which should have been persisted was NOT found in the DB. " + tasks, emProvider.get()
    ...
    public void assertNoEntityHasBeenPersisted() {
        checkState(!storedEntities.isEmpty(), "no entities to check");
        for (TestEntity storedEntity : storedEntities) {
            assertNull("At least one entity which should NOT have been persisted was found in the DB. " + tasks, emProvider.get()
                                                                                                                           .find(TestEntity.class,
                                                                                                                                   storedEntity.getId()));
        }
    }
    ...
    private static class TransactionalTasks {
         ...
         *         if {@link #next()} has already been called on this instance.
         */
